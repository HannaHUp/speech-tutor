...
Phase 1 is a localhost voice loop:
browser mic -> STT -> LLM -> TTS -> playback.

The app is served by FastAPI on `http://localhost:8000`. FastAPI serves the built Svelte app from `web/dist`, so the full app starts from the Python backend, not from Vite alone.
